...
lala <- linearitaet_filter(linearitaet_prediction = fao$pred.lin[[i]]$prediction
                   , ncomp = fao$plspara$ncomp
                   , linearitaet_limit_1 = 4.465 * .9
                   , linearitaet_limit_2 = 5.456 * 1.1
                   , R_2 = .9
                   , SOLL = fao$lin$data$Acid)

fao$plspara$wl <- lala[ , 2:5][!duplicated(lala[ , 2:5]),]
...
fao$pred.lin.diff
fao$pred.lin.diff[[1]][ order( fao$pred.lin.diff[[1]]$spcR), ]
